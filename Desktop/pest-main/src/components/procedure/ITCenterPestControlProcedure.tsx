...
const ProcedureStep = ({ icon, title, description, isOpen, toggleStep, stepNumber }: ProcedureStepProps) => {
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      <div
        className={`flex items-center justify-between p-4 cursor-pointer transition-all duration-300 ${isOpen ? 'bg-cyan-50/50' : 'bg-white'}`}
        onClick={toggleStep}
      >
        <div className="flex items-center gap-3 flex-1">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 ${isOpen ? 'bg-cyan-600 text-white' : 'bg-cyan-100 text-cyan-600'}`}>
            {icon}
          </div>
          <h3 className={`font-semibold text-lg transition-colors duration-300 ${isOpen ? 'text-cyan-700' : 'text-gray-800'} flex-1`}>
            {title}
          </h3>
        </div>
...
export default function ITCenterPestControlProcedure() {
  ...
  const procedureSteps = [
    ...
    },
    {
      ...
      title: "Equipment-Safe Protocol Development",
  ...
  return (
    <section className="py-16 bg-gray-50 relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-cyan-50 rounded-bl-[100px] z-0"></div>
      <div className="absolute bottom-0 left-0 w-1/4 h-1/4 bg-cyan-50 rounded-tr-[100px] z-0"></div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl font-bold mb-4 text-gray-900">
            Our IT Center <span className="text-cyan-600">Pest Control</span> Process
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="relative rounded-xl overflow-hidden shadow-xl h-[400px] md:h-[500px] group">
            <div className="absolute inset-0 p-0.5 rounded-xl bg-cyan-500 z-10 opacity-70 group-hover:opacity-100 transition-opacity duration-300"></div>

            <div className="absolute inset-0.5 rounded-xl overflow-hidden z-20">
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent z-10"></div>
              <Image
                src="/it-center-pest-control.jpg"
                alt="IT Center Pest Control Process"
                fill
            ...
            <div className="absolute bottom-0 left-0 p-6 z-30">
              <h3 className="text-white font-bold text-xl mb-2">Technology-Safe Pest Management</h3>
              <p className="text-white/80 max-w-xs">Our IT center pest control services protect sensitive equipment while effectively managing pest issues in technology environments.</p>
            </div>
          </div>
        ...
        <div className="mt-12 text-center">
          <div className="bg-cyan-600 text-white p-6 rounded-xl shadow-lg max-w-2xl mx-auto transform hover:scale-[1.02] transition-transform duration-300 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="text-xl font-semibold mb-3">Protect your IT infrastructure with professional pest control</h3>
              <p className="text-white/80 mb-4">Maintain operational continuity and equipment integrity with our specialized IT center pest management services.</p>
              <a href="/contact" className="inline-block bg-white text-cyan-600 hover:text-cyan-700 font-bold py-3 px-8 rounded-full transition-all duration-300 border-2 border-white hover:border-opacity-80 hover:shadow-lg">
