...
procedureFiles.forEach(file => {
  ...
  if (content.includes('ProcedureStep') && content.includes('stepNumber')) {
    ...
    // Update the step title to be properly aligned
    updatedContent = updatedContent.replace(
      /<h3 className={`font-semibold text-lg transition-colors duration-300 \${isOpen \? '([^']+)' : '([^']+)'}`}>/g,
      '<h3 className={`font-semibold text-lg transition-colors duration-300 ${isOpen ? \'$1\' : \'$2\'} flex-1`}>'
    );
    
    // Update the flex container for better alignment
    updatedContent = updatedContent.replace(
